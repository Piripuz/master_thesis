...
\subsubsection{Normalizing the paramter $\alpha$}
\label{sec:norm_alpha}
...
\subsubsection{Expressing Travel Time in terms of Arrival Time}
\label{sec:t_a_t_d}

Note now that most of the cost function depend on when the user \textit{arrives} rather than when the user decides to leave.
...
(it is impossible that, by departing later, a commuter arrives earlier or at the same time),
...
These bounds will be always satisfied for real travel time functions,
